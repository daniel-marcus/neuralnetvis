...
export function useAnimatedPosition(position: number[], speed = 0.4) {
  ...
  useFrame(() => {
    if (!ref.current || targetPos.equals(currentPosition)) return
    invalidate()
    currentPosition.lerp(targetPos, speed)
    if (currentPosition.distanceTo(targetPos) < 0.01) {
      currentPosition.copy(targetPos) // allow tolerance for floating point errors
    } else {
